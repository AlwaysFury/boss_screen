...
//    }
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public void refreshEscrowInfoBySn(List<List<String>> orderSnLists, long shopId) {
        List<EscrowInfo> escrowInfoList =  new ArrayList<>();

        log.info("===支付信息发送请求及处理开始");
        long startTime =  System.currentTimeMillis();

        for (List<String> orderSnList : orderSnLists) {
...
//        CompletableFuture.allOf(insertEscrowItemFutures.toArray(new CompletableFuture[0])).join();
    ...
    public void saveEscrowInfoByOrderSn(JSONObject orderIncomeObject, String orderSn, List<EscrowInfo> escrowInfoList, long shopId) {
        ...
        EscrowInfo escrowInfo = EscrowInfo.builder()
                ...
                .sellerReturnRefund(orderIncomeObject.getBigDecimal("seller_return_refund"))
                .shopeeDiscount(orderIncomeObject.getBigDecimal("shopee_discount"))
                .voucherFromSeller(orderIncomeObject.getBigDecimal("voucher_from_seller"))
                .reverseShippingFee(orderIncomeObject.getBigDecimal("reverse_shipping_fee"))
                .orderAmsCommissionFee(orderIncomeObject.getBigDecimal("order_ams_commission_fee"))
                .commissionFee(orderIncomeObject.getBigDecimal("commission_fee"))
                .serviceFee(orderIncomeObject.getBigDecimal("service_fee"))
                .sellerTransactionFee(orderIncomeObject.getBigDecimal("seller_transaction_fee"))
                .exchangeRate(orderIncomeObject.getDouble("exchange_rate") == null ? 1.0 : orderIncomeObject.getDouble("exchange_rate"))
                .shopId(shopId)
                .build();
